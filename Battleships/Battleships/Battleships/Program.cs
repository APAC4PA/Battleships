...
namespace Battleships {
	internal class Program {
		...
		static void PlacingTurn(Player player) {
			...
			Console.WriteLine(placingShips);
			SelectShips.InsertShips(out coordinateX, out coordinateY, out shipSize, out shipRotation);
			Console.WriteLine($"x: {coordinateX}, y: {coordinateY}");
			isOptionInvalid = player.PlaceShips(shipSize, shipRotation, coordinateY, coordinateX);
			if (!isOptionInvalid) {
				if (player.shipsToPlace[shipSize - 1] > 0) {
					player.shipsToPlace[shipSize - 1]--;
					placingShips++;
				} else {
			...
			player.PlaceShips(shipSize, shipRotation, coordinateY, coordinateX);
			Console.WriteLine("Your ships");
			player.boardShips.ShowBoard();
